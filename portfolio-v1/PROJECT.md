...
André Finageiv's personal design portfolio. Editorial-luxury aesthetic inspired by Italian fashion case sites. Six case studies (four full + two light) + About + Contact + Deep-dive.
...
- **Removed Fit4Box** case entirely from `data.js`. Left `assets/bg-fit4box.jpg` and its now-dead `identity`/`lookbook`/`pdp`/`campaign`/`packaging` plate-factory branches in place — harmless.
- **Enriched Farsight** (`data.js`): abstract/approach copy now names the 3-layer token architecture, 24 production components, and the KPI-page pattern (Turbulence Intensity as reference template, reused for Wind Curtailment/NTF/Solar Analysis). Swapped 2 of 4 plates for real screenshots: `assets/farsight/ti-over-time.png` + `ti-power-curve.png` (cropped from existing `Projects/Farsight/Turbulence-Intensity/crop_*.png`) and `assets/farsight/design-system.png` (freshly captured via Playwright CLI from the live Design-System token/component viewer). Kept the `system-overview` SVG plate.
- **Enriched Grid-Ops**: abstract/approach now name the 12-column widget grid and 9 chart-type system. Swapped 2 of 4 plates for real screenshots: `assets/grid-ops/dashboard-widgets.png` (captured from `Projects/Farsight/Dashboard-Widgets/`) and `assets/grid-ops/dashboard-1.png` (from 3 real product screenshots in `Projects/Farsight/Dashboard/`, cropped ~13% off the top via PIL to remove browser-chrome/tab-bar that was in the original captures). Kept `before-after` and `asset-detail` SVG plates.
- **Added new case: Financial Losses** (`num: '003'`, inserted after Grid-Ops — Growth Funnel renumbered `003`→`004`, copy unchanged). Built from `Projects/Farsight/Financial-Losses/FinancialLosses-Research.md` — primary frame is the loss-taxonomy-as-differentiator angle (bridging IEC 61400-15 pre-construction and 61400-26 operational standards into one 8-category ledger), with Data Quality (4-label trust model) and Remote Control (audit trail / Select-Before-Operate safety) referenced as supporting context, not co-equal. KPIs are directional/scope counts (8 categories, 3 standards bridged, 6 competitors benchmarked, 3 differentiation angles) — **no shipped-metric numbers exist yet**; flagged to André as an open item if he later wants to swap in real figures. Two new plate-factory SVG kinds added to `data.js`: `loss-waterfall` (descending IEC 61400-15 waterfall bars) and `availability-gap` (3-bar Technical/Operational/Contractual comparison illustrating the 1–2% dispute gap). No hero image (`hero: null`) — deliberately asset-poor/research-stage, no real screenshots exist for this module yet.
- **Added Erico Adv as a light case** (`num: '005'`, `light: true`). Hero = `assets/bg-erico.jpg` (copied from `Projects/Erico Adv/Web/assets/bg-hero.jpg`, real brand photography, no capture needed). CTA links to `ericoadv.vercel.app` (current staging — André confirmed OK to link despite it being noindex'd elsewhere).
- **Added Destiny-Pixel as a light case** (`num: '006'`, `light: true`). Hero = `assets/bg-destiny-pixel.png`, captured via Playwright CLI from the live title screen at `destiny-pixel.vercel.app`; cropped the right-side patch-log panel off and the top ~110px off (the game's own "DESTINY PIXEL" title text collided visually with the portfolio's own breadcrumb overlay in the same corner — cropping the title text out of the source image fixed it). CTA links to `destiny-pixel.vercel.app`.
- **Updated hub tile** at root `index.html` — Portfolio 2026 card description changed from "four cases" to "four cases, two light cases" to match the new count.
- **Verified**: all 6 tabs cycle via click and confirmed no console errors at any case; both light cases confirmed to render the external-link CTA (not the deep-dive button), and confirmed Enter/wheel-scroll-to-bottom do NOT attempt to open a deep-dive (no `<DeepDive>` in the DOM for light cases) and arrow-up/down don't crash; checked 1280/1440/mobile (375×812) — 6 tabs fit on one row at desktop widths and stack cleanly on mobile, no overflow, no new responsive rule needed.
- **Found and discarded an orphaned uncommitted edit** to the (now-relocated) root `PROJECT.md` at session start — a "Session 2026-04-20 (continued)" entry describing two unimplemented viewport-transition variants (Lime/Slash) with no corresponding code anywhere in the working tree, and not present in origin's `portfolio/PROJECT.md` either. Backed up the diff to the session scratchpad before discarding, in case it's wanted later.
...
- ~~Mobile (≤900px): SCROLL cue overlaps right-edge KPI; console bleeds under fixed statusbar when scrolled long.~~ — **resolved 2026-07-05**: scroll-cue was already hidden on mobile (dead code path, confirmed harmless) and the statusbar padding already cleared. Found and fixed three related, previously-undocumented mobile bugs in the same pass: statusbar text wrapping out of its 28px band (added `white-space: nowrap`, moved coordinates/FILE readout behind `.sb-coords`/`.sb-file` classes hidden ≤900px), stray `.project-tabs .sep` hairlines floating mid-row once tabs wrap to one-per-row, and a CSS Grid blowout in `.cal-strip` (`repeat(7, 1fr)` → `repeat(7, minmax(0, 1fr))`) that was forcing the entire `.availability` grid — including the description paragraph — to overflow to 444px inside a 335px container, clipping Friday/Saturday off-canvas with no way to reach them.
- ~~A11y (for strict WCAG AA): no page-level `<h1>` anywhere; Tweaks panel range input lacks `for`/`id` association.~~ — **resolved 2026-07-05**: About and Contact already had their own `<h1>`; the Work view (the site's default/home view) did not — `Console.jsx`'s `.case-name` promoted from `<h2>` to `<h1>` (DeepDive's `<h2>` project title stays subordinate to it, correct hierarchy). Tweaks panel: added `id="tw-font-scale"` on the range input and matching `htmlFor` on its label.
- ~~No favicon / OG image currently — need a new treatment now that AFMark is gone.~~ — **resolved 2026-07-05**: new "Instrument Panel" reticle mark (viewfinder corner-brackets + center signal dot, `favicon.svg` at hub root) replaces the retired AFMark everywhere, including a stray AF-monogram data-URI favicon that had crept back into `folio/index.html`. New 1200×630 OG image (`assets/og-image.png`) wired into `og:image`/`twitter:image` across index.html, portfolio, folio, fluted-glass, marathon-preview.
