...
var _cardAnimSlotId = null;
var _cardAnimating  = false;
...
function _populateModal(id) {
  ...
  $('pdShinyTag').style.display = isShiny ? 'block' : 'none';
...
function _buildCardFront(id) {
  var captured = state.pokedex[id];
  var isShiny  = !!captured.shiny;
  var front = document.querySelector('.pca-front');
  front.innerHTML =
    '<div class="pca-num">#' + pad3(id) + '</div>' +
    '<div class="pca-img"><img src="' + spriteUrl(id, isShiny) + '" alt=""></div>' +
    '<div class="pca-name">' + getPokemonName(id) + '</div>';
  front.style.background = isShiny
    ? 'linear-gradient(135deg,#FEF3C7,#FBBF24)'
    : 'white';
  front.style.borderColor = isShiny ? '#F59E0B' : 'var(--yellow)';
}

function _getSlotRect(id) {
  var slot = document.querySelector('.pokedex-slot[data-id="' + id + '"]');
  return slot ? slot.getBoundingClientRect() : null;
}

function _showModalInstant(id) {
  _populateModal(id);
  var overlay = $('pokemonDetailOverlay');
...
function openPokemonModal(id) {
  if (!state.pokedex[id] || _cardAnimating) return;
  _cardAnimating  = true;
  _cardAnimSlotId = id;
  ...
  var slotRect = isMobile ? null : _getSlotRect(id);

  _buildCardFront(id);

  var card  = $('pokemonCardAnim');
  var inner = card.querySelector('.pca-inner');

  var vw = window.innerWidth;
  var vh = window.innerHeight;
  var targetW = Math.min(vw - 32, 430);
  var targetH = Math.min(vh * 0.88, 560);
  var targetLeft = (vw - targetW) / 2;
  var targetTop  = (vh - targetH) / 2;

  // Reset any previous transform state
  ...
  function afterFlip() {
    ...
    // Suppress built-in CSS animations — we control the reveal
    overlay.style.animation = 'none';
    if (modal) modal.style.animation = 'none';
    overlay.style.opacity = '0';
    overlay.style.display = 'flex';

    anime.animate(overlay, { opacity: [0, 1], duration: 220, ease: 'linear' });
    anime.animate(card,    { opacity: [1, 0], duration: 220, ease: 'linear' }).then(function() {
  ...
  if (slotRect) {
    // Desktop: fly from slot to center
    var dx = targetLeft - slotRect.left;
    var dy = targetTop  - slotRect.top;
    var sx = targetW / slotRect.width;
    var sy = targetH / slotRect.height;

    card.style.cssText =
      'display:block;left:' + slotRect.left + 'px;top:' + slotRect.top + 'px;' +
      'width:' + slotRect.width + 'px;height:' + slotRect.height + 'px;' +
      'border-radius:8px;opacity:1;';

    var tl = anime.createTimeline({ defaults: { ease: 'inOutQuart' } });
    ...
    card.style.cssText =
      'display:block;left:' + targetLeft + 'px;top:' + (vh + 20) + 'px;' +
      'width:' + targetW + 'px;height:' + targetH + 'px;' +
      'border-radius:16px;opacity:1;';

    var tl2 = anime.createTimeline({ defaults: { ease: 'outQuart' } });

    tl2.add(card, {
      translateY: [0, -(targetH + 20 + (vh - targetH) / 2)],
      duration:   500,
    }, 0);
...
function closePokemonModal() {
  ...
  // Start card at modal center position with back face visible
  card.style.cssText =
    'display:block;left:' + targetLeft + 'px;top:' + targetTop + 'px;' +
    'width:' + targetW + 'px;height:' + targetH + 'px;' +
    'border-radius:16px;opacity:1;';
  ...
  function afterClose() {
    ...
    inner.style.transform = '';
    _cardAnimating = false;
    _cardAnimSlotId = null;
  }

  if (slotRect) {
    var dx = slotRect.left - targetLeft;
    var dy = slotRect.top  - targetTop;
    var sx = slotRect.width  / targetW;
    var sy = slotRect.height / targetH;
    ...
    // Mobile: slide back down
    var tl2 = anime.createTimeline({ defaults: { ease: 'inOutQuart' } });
